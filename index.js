// index.js (aggiornato) – 2025‑06‑15
//  • Weather.com/PWS ogni 10 minuti
//  • Open‑Meteo ogni 30–40 minuti (intervallo random)
//  • Batch fino a 100 coordinate Open‑Meteo per ridurre le chiamate
//  • Richiede solo temperatura e umidità (2 variabili) per stare <10k unit/giorno
...
// ---------------- CONFIG ----------------
const WEATHERCOM_INTERVAL_MIN = 10;      // PWS
const OPENMETEO_INTERVAL_MIN_MIN = 30;   // 30–40 min finestra
const OPENMETEO_INTERVAL_MIN_MAX = 40;
const MAX_LOCATIONS_PER_CALL = 100;
const OPENMETEO_PARAMS = 'temperature_2m,relative_humidity_2m';
...
async function fetchWeatherComAll() {
  const pws = stazioni.filter(s => s.apiKey);
  for (const st of pws) {
...
async function fetchOpenMeteoBatch(batch) {
  ...
  for (let i = 0; i < records.length; i++) {
    const st = batch[i];
    const cur = records[i]?.current;
    if (!cur) {
      console.warn('Open‑Meteo: dati assenti per', st.stationId);
      continue;
    }
    await salvaOsservazione(
      st.stationId,
...
async function fetchOpenMeteoAll() {
  const list = stazioni.filter(s => s.openMeteo);
  const batches = chunkArray(list, MAX_LOCATIONS_PER_CALL);

  for (const batch of batches) {
    try {
      await fetchOpenMeteoBatch(batch);
    } catch (err) {
      console.error('Errore batch Open‑Meteo:', err.message);
      if (batch.length > 1) {
...
async function mainCycle() {
  ...
  if (Date.now() >= nextOpenMeteo) {
    await fetchOpenMeteoAll();
    const nextMin = OPENMETEO_INTERVAL_MIN_MIN +
        Math.floor(Math.random() * (OPENMETEO_INTERVAL_MIN_MAX - OPENMETEO_INTERVAL_MIN_MIN + 1));
    nextOpenMeteo = Date.now() + nextMin * 60 * 1000;
    console.log(`Open‑Meteo prossimo aggiornamento fra ${nextMin} minuti`);
  }
}

// ---------- AVVIO ----------
await mainCycle();
setInterval(mainCycle, WEATHERCOM_INTERVAL_MIN * 60 * 1000);
